            ...
            {table.getHeaderGroups().map((headerGroup) => (
              ...
              >
                {headerGroup.headers.map((header) => (
                  <TableHead
                    key={header.id}
                    className={`px-3 py-2.5 h-9 text-[11px] font-normal text-muted-foreground/70 whitespace-nowrap ${
                      header.id === "exam_date"
            ...
            {table.getRowModel().rows.length > 0 ? (
              table.getRowModel().rows.map((row) => (
                <TableRow
                  ...
                  className={`cursor-pointer transition-colors border-b border-border/20 last:border-0 hover:bg-foreground/5`}
                >
